    ...
    return (
      <div className="flex flex-col h-full overflow-y-auto p-6">
        <div className="max-w-2xl mx-auto w-full">
          ...
          {/* Failed Question Reviews */}
          {results.reviews && results.reviews.length > 0 && (
            <div className="bg-darkCard rounded-lg p-6 border border-gray-700 mb-6">
              <h3 className="text-lg font-semibold mb-3">🔍 Review Failed Questions</h3>
              <div className="space-y-4">
                {results.reviews.map((r, idx) => (
                  <div key={idx} className="p-4 bg-gray-800 rounded-lg">
                    <div className="text-sm text-gray-300 mb-2">{r.question}</div>
                    <div className="text-xs text-gray-400 mb-2">Correct answer: <span className="text-primary">{r.correct_option}</span></div>
                    {r.explanation && <div className="text-xs text-gray-400 mb-2">Explanation: {r.explanation}</div>}
                    <div className="text-sm font-semibold mb-1">Next steps:</div>
                    <ul className="list-disc list-inside text-gray-300">
                      {r.suggested_next_steps.map((s, i2) => (
                        <li key={i2}>{s}</li>
                      ))}
                    </ul>
                  </div>
